...
namespace kinematic_viewer {
    ...
    class KinematicInputHandler {
       public:
        ...
        LinkPickResult UpdateLinkPick(const UpdateContext& ctx, const glm::mat4& view, const glm::mat4& proj, rkv::RobotScene* scene);

        // Raycast link under cursor (fast proxy pick). When unthrottled=true, updates every frame.
        LinkPickResult UpdateLinkHover(const UpdateContext& ctx, const glm::mat4& view, const glm::mat4& proj, rkv::RobotScene* scene,
                                       double now_sec, bool unthrottled = false);
        ...
        ViewportHotkeyResult HandleViewportHotkeys(bool enable_hotkeys, bool has_playable_trajectory, float dt_sec, float play_speed);
